...
const server = new Server(
  { name: 'agentscope-sift', version: '1.0.0' },
  { capabilities: { tools: {} } }
)
...
async function dispatch(name: string, a: Record<string, string | number>): Promise<object> {
  switch (name) {
    ...
    case 'list_files':              return SIFT.list_files(String(a.image_path), a.offset as number | undefined)
    case 'get_filesystem_info':     return SIFT.get_filesystem_info(String(a.image_path), a.offset as number | undefined)
    case 'scan_with_yara':          return SIFT.scan_with_yara(String(a.rules_file), String(a.target_path))
    case 'extract_strings':         return SIFT.extract_strings(String(a.file_path), a.min_length as number | undefined)
