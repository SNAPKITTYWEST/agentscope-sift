...
async function safeExec(bin: string, args: string[]): Promise<string> {
  ...
  try {
    ...
    if (err.code === 'ENOENT') return `[TOOL NOT FOUND: ${bin} — install SIFT workstation]`
...
export async function list_processes(memory_image: string): Promise<object> {
...
export async function scan_network_connections(memory_image: string): Promise<object> {
  requireFile(memory_image)
...
export async function dump_cmdline(memory_image: string): Promise<object> {
  requireFile(memory_image)
...
export async function find_injected_code(memory_image: string): Promise<object> {
  requireFile(memory_image)
...
export async function get_registry_hives(memory_image: string): Promise<object> {
  requireFile(memory_image)
...
export async function list_disk_partitions(image_path: string): Promise<object> {
...
export async function list_files(image_path: string, offset?: number): Promise<object> {
  requireFile(image_path)
...
export async function get_filesystem_info(image_path: string, offset?: number): Promise<object> {
  requireFile(image_path)
  const args = offset !== undefined ? ['-o', String(offset), image_path] : [image_path]
...
export async function build_supertimeline(evidence_path: string, output_file: string): Promise<object> {
...
export async function query_timeline(plaso_file: string, query: string): Promise<object> {
...
export async function scan_with_yara(rules_file: string, target_path: string): Promise<object> {
  if (!existsSync(rules_file)) throw new Error(`YARA rules file not found: ${rules_file}`)
...
export async function extract_strings(file_path: string, min_length = 8): Promise<object> {
...
export async function hash_file(file_path: string): Promise<object> {
  requireFile(file_path)
...
export async function get_file_metadata(file_path: string): Promise<object> {
  requireFile(file_path)
  ...
  let parsed: object = {}
  try { parsed = JSON.parse(meta)[0] ?? {} } catch {}
  return { tool: 'exiftool', file: file_path, file_type: fileType.split(':').slice(1).join(':').trim(), metadata: parsed }
}

// ── Network Forensics ────────────────────────────────────────────────────────

export async function analyze_pcap(pcap_file: string): Promise<object> {
...
export async function get_system_info(): Promise<object> {
