...
async function splunkOK(promise: Promise<boolean>, label: string) {
  const ok = await promise
  if (ok) log(DIM, 'SPLUNK', `→ ${label} indexed`)
}
...
async function runDemo() {
  ...
  await splunkOK(splunk.finding(
    'svchost.exe (PID 1337) spawned cmd.exe — anomalous parent-child',
    0.91, 'confirmed', entry.seq
  ), 'finding:confirmed')
  ...
  await splunkOK(splunk.agentDisagreement(
    'check_autoruns', 'PASS', 'QUARANTINED',
    'Conservative path: treat as QUARANTINED — SENTINEL has domain-specific LOLBin pattern knowledge'
  ), 'agent_disagreement → SOC alert')
  ...
  await splunkOK(splunk.councilEscalation(
    'Agent disagreement + confidence 0.42 — LOLBin masquerade pattern requires human triage',
    lowConf,
    'svchost32.exe in HKCU\\Run\\SystemUpdate — possible persistence mechanism'
  ), 'council_escalation → HIGH priority alert')
